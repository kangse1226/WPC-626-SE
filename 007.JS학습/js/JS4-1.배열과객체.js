...
/**********************************************
    ...
    이를 인스턴스(instance)라고 함! 원본에서 객체 가져와서 기능만 갖다쓰는거!(피그마 컴포넌트>인스턴스 그거)

    2. 리터럴 선언방식 (배열리터럴) : 이쪽을 자주씀
    - 변수 = []
...
const 김태리 = new Array(
  ...
  [2014, "더바디샵"],
  ["리틀 포레스트", "정년이"],
  //배열안에 함수쓰기- 6번째는 클릭시 호출
  function () {
    alert("안녕하세요, 배우 김태리입니다!");
},
    //7번째는 마우스오버시/ 아웃시 호출
    (보일락말락)=>{
        console.log("이상 김태리 배우였습니다!",보일락말락, 김태리이미지);
    // 전역변수인   `김태리이미지` 를 보였다 안보였다 CSS 변경하기
    김태리이미지.style.display = 보일락말락; 
}
    

); ///김태리 배열///

//출력박수에 클릭할 경우 함수 실행하기
출력박스[0].onclick = 김태리[6];

//만약 함수를 할당할 때 값을 보내야하면
//익명함수에 넣어서 값을 보내는 세팅을 하고, 할당하면 된다!
...
// 1. 변수.item(순번)
//2. 변수[순번]
//주의!! 기존의 이미지태그를 살리려면 +=로 데이터를 추가하는 코딩을 해야함!
출력박스[0].innerHTML += `
배우: ${김태리[0]} /
생년월일: ${김태리[1]}/
키: ${김태리[2]}/
...
//위에서 김태리 이미지를 만든 후에! html을 모두 완성한 다음!!
//  변수에 할당한다. 
//이 순서가 매우매우매우 매우매우매우 중요하다!!!!!!!!!!!!
const 김태리이미지 = document.querySelector("#kim");
//->만약 html을 내부에 다시 추가하게되면, 
//이미 변수에 담긴 html요소가 연결이 끊어진다! 
//왜?? 새로운 구조로 변경되었기 때문이다!!!!!!!!!!!!!!!!!!!!!

// console.log('태리이미지 어딨니?', 김태리이미지); 확인했으니 주석달기


// 1-2. 배열 리터럴 방식의 선언과 할당 ///////////
// 배열변수명 = [값1, 값2, 값3, ...];
// new 키워드 없이 바로 쓸 수 있는 객체임!
//객체는 다 준비되어있는 거임. 완제품. 예를들면 자동차. 자동차는 다 준비된거를 구매하는거잖아. 내가 구매해서 핸들을 끼우진 않지. 
//이런 객체를 정적개체(Static Object)이라고 한다! 
...
const 삼일절 = [
 ...
 // 4. 4초후에 다시 원래 크기로 돌아가기
    ...
    // 화살표함수가 아니라 function(){}일 경우
    // this키워드가 4초후 전달되지 않는다.
    // 전달하기 위해 bind()함수를 사용해야함! .bind(this)

  },  // 함수// 
  //원래 배열끝에 콤마를 쓰면 에러였으나 
  // 최신브라우저 문법에서는 허용하고 있음!
];
...
//title 툴팁 넣기
출력박스[1].title = "클릭하면 만세를 합니다!";

//마우스 포인터를 손가락 모양으로 설정하기
